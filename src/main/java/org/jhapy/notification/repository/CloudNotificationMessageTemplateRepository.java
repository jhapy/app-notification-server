 ...
 * @since 2019-02-22
 */
public interface CloudNotificationMessageTemplateRepository
    extends MongoRepository<CloudNotificationMessageTemplate, UUID> {

  @Query(
      "{'$and' : [{'$or' : [{'name' : {$regex : ?0, $options: 'i'}}, {'title' : {$regex : ?0, $options: 'i'}}, {'body' : {$regex : ?0, $options: 'i'}}, {'data' : {$regex : ?0, $options: 'i'}}]}, {'isActive' : ?1}]}")
  Page<CloudNotificationMessageTemplate> findByCriteriaAndIsActive(
      String criteria, Boolean isShowInactive, Pageable pageable);

  @Query(
      value =
          "{'$or' : [{'name' : {$regex : ?0, $options: 'i'}}, {'title' : {$regex : ?0, $options: 'i'}}, {'body' : {$regex : ?0, $options: 'i'}}, {'data' : {$regex : ?0, $options: 'i'}}]}")
  ...
  Optional<CloudNotificationMessageTemplate>
