 ...
 * @since 2019-02-22
 */
public interface SmsMessageRepository extends MongoRepository<Sms, UUID> {

  @Query(
      "{'$and' : [{'$or' : [{'body' : {$regex : ?0, $options: 'i'}}, {'phoneNumber' : {$regex : ?0, $options: 'i'}}]}, {'isActive' : ?1}]}")
  Page<Sms> findByCriteriaAndIsActive(String criteria, Boolean isShowInactive, Pageable pageable);

  @Query(
      "{'$or' : [{'body' : {$regex : ?0, $options: 'i'}}, {'phoneNumber' : {$regex : ?0, $options: 'i'}}]}")
