 ...
 * @since 2019-02-22
 */
public interface SmsTemplateRepository extends MongoRepository<SmsTemplate, UUID> {

  @Query(
      "{'$and' : [{'$or' : [{'name' : {$regex : ?0, $options: 'i'}}, {'body' : {$regex : ?0, $options: 'i'}}]}, {'isActive' : ?1}]}")
  Page<SmsTemplate> findByCriteriaAndIsActive(
      String criteria, Boolean isShowInactive, Pageable pageable);

  @Query(
      value =
          "{'$or' : [{'name' : {$regex : ?0, $options: 'i'}}, {'body' : {$regex : ?0, $options: 'i'}}]}")
