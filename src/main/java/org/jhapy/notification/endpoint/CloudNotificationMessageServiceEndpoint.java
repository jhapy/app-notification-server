...
public class CloudNotificationMessageServiceEndpoint extends BaseEndpoint {
  ...
  public ResponseEntity<ServiceResult> findAnyMatching(@RequestBody FindAnyMatchingQuery query) {
    ...
    Page<org.jhapy.notification.domain.CloudNotificationMessage> result =
        cloudNotificationMessageService.findAnyMatching(
            query.getFilter(), converter.convert(query.getPageable()));
    return handleResult(
        loggerPrefix,
        toDtoPage(
            result, getConverter().convertToDtoCloudNotificationMessages(result.getContent())));
  }
  ...
  public ResponseEntity<ServiceResult> countAnyMatching(@RequestBody CountAnyMatchingQuery query) {
    ...
    return handleResult(
        loggerPrefix, cloudNotificationMessageService.countAnyMatching(query.getFilter()));
  }
