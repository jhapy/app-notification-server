...
public class SmsServiceEndpoint extends BaseEndpoint {
  ...
  public SmsServiceEndpoint(SmsService smsService, NotificationConverterV2 converter) {
    super(converter);
  ...
  public ResponseEntity<ServiceResult> findAnyMatching(@RequestBody FindAnyMatchingQuery query) {
    ...
    Page<Sms> result =
        smsService.findAnyMatching(query.getFilter(), converter.convert(query.getPageable()));
    return handleResult(
        loggerPrefix, toDtoPage(result, getConverter().convertToDtoSmss(result.getContent())));
  }

  @PostMapping(value = "/countAnyMatching")
  public ResponseEntity<ServiceResult> countAnyMatching(@RequestBody CountAnyMatchingQuery query) {
    var loggerPrefix = getLoggerPrefix("countAnyMatching");
    return handleResult(loggerPrefix, smsService.countAnyMatching(query.getFilter()));
  }
